...
def msg_f(msg: dict):
    ...
    for i in values:
        ...
        embed.add_field(name=i[0], value=i[1], inline=inline)
...
class Log(commands.Cog):
    ...
    async def log(self, ctx, i=None):
        ...
        if not logs:
            ...
            r = logs.find_one({"_id": 0})
        if str(ctx.message.guild.id) not in logs:
            logs.insert_one({"_id": 0})
            r = logs.find_one({"_id": 0})
        ...
        category = await ctx.message.guild.create_category(
            "Logs", reason=None, overwrites=overwrites)

        # log chat; edit, delete message
        chatL = await ctx.message.guild.create_text_channel(f'chat log',
                                                            category=category)

        # log role; add, remove, edit role
        roleL = await ctx.message.guild.create_text_channel(f'role log',
                                                            category=category)

        # log voice;swich vc, join vc, leave vc
        vcL = await ctx.message.guild.create_text_channel(f'vc log',
                                                          category=category)
        # log banned; banned, unbanned, kicked
        bannedL = await ctx.message.guild.create_text_channel(
            f'kick ban log', category=category)

        # log channel; edited, deleted, added channels
        channelsL = await ctx.message.guild.create_text_channel(
            f'channel log', category=category)

        # log member; nickname
        memberL = await ctx.message.guild.create_text_channel(
            f'member log', category=category)
        ...
        embed = nextcord.Embed(
            title="",
            description="LOG category and its channels\nCreated",
            color=0x00ff00)
    ...
    async def on_member_remove(self, member):
        ...
        if str(member.guild.id) in log:
            async for entry in member.guild.audit_logs(
                    limit=1, action=nextcord.AuditLogAction.kick):
                entery = entry
            async for entryy in member.guild.audit_logs(
                    limit=1, action=nextcord.AuditLogAction.ban):
                enteryy = entryy
            if entery.target.id == member.id and abs(entery.created_at -
                                                     now).total_seconds() < 2:
                ...
                embed = msg_f({
                    ...
                    [["Member:", f"{member.name} | {member.id}"],
                     ["Kicked By:", f"{entery.user.name} | {entery.user.id}"],
        ...
        try:
            if str(before.guild.id) in log:  # nickname changed
                if before.display_name != after.display_name:
                    ...
                    async for entry in before.guild.audit_logs(
                            limit=1,
                            action=nextcord.AuditLogAction.member_role_update):
                        entery = entry
                    o = False
            ...
            if str(before.guild.id) in log:
                if before.name != after.name:
                    ...
                    embed = msg_f({
                        ...
                        "value": [["Old:", f"{before.discriminator}"],
    ...
    async def on_guild_role_create(self, role):
        db = pymongo.MongoClient(getenv["mongoDBclient"])[role.guild.id]
        logs = db["logs"]
        log = logs.find_one({"_id": 0})
        if not log:
            logs.insert_one({"_id": 0})
        ...
        if str(role.guild.id) in log:
            ...
            async for entry in role.guild.audit_logs(
                    limit=1, action=nextcord.AuditLogAction.role_create):
                entery = entry
            embed = msg_f({
                "title":
                "Role created",
    ...
    async def on_guild_role_delete(self, role):
        db = pymongo.MongoClient(
            getenv["mongoDBclient"])[role.message.guild.id]
        logs = db["logs"]
        log = logs.find_one({"_id": 0})
        if not log:
            logs.insert_one({"_id": 0})
        ...
        if str(role.guild.id) in log:
            log_c = log[str(role.guild.id)]["role"]
            async for entry in role.guild.audit_logs(
                    limit=1, action=nextcord.AuditLogAction.role_delete):
                entery = entry
            embed = msg_f({
                "title":
                "Role deleted",
    ...
    async def on_guild_role_update(self, before, after):
        ...
        try:
            if str(before.guild.id) in log:
                async for entry in before.guild.audit_logs(
                        limit=1, action=nextcord.AuditLogAction.role_update):
                    entery = entry
                ...
                log_c = log[str(before.guild.id)]["role"]
                r = [
                    perm[0].replace("_", " ").replace("guild", "server")
                ...
                if after.name == before.name:
                    s.append(["Role Name:", f"{before.name} | {before.id}"])
                else:
                    s.append(["Old name:", f"{before.name} | {before.id}"])
                    s.append(["New name:", f"{after.name} | {after.id}"])
                s.append([
                    "Added Permissions",
                    ", ".join(a if len(a) != 0 else ["No added perms"]), False
                ])
                s.append([
                    "Removed Permissions",
                    ", ".join(r if len(r) != 0 else ["No removed perms"])
                ])

                msg = {"title": "Role edited", "value": s}
        ...
        if str(check_l.guild.id) in log:
            ...
            if before.channel is None and after.channel is not None:
                msg = {
                    ...
                    "Join VC",
                    "value":
                    [["Member", f"{member.name} | {member.id}", False],
                     [
                         "Joining to: ",
    ...
    async def on_member_ban(self, guild, user):
        db = pymongo.MongoClient(getenv["mongoDBclient"])[str(guild.id)]
        logs = db["logs"]
        log = logs.find_one({"_id": 0})
        if not log:
            logs.insert_one({"_id": 0})
        ...
        if str(guild.id) in log:
            ...
            embed = msg_f({
                ...
                [["Member:", f"{user.name} | {user.id}"],
                 ["Banned By:", f"{entery.user.name} | {entery.user.id}"],
                 ["Reason:", f"{entery.reason}", False]]
            })
    ...
    async def on_member_unban(self, guild, user):
        ...
        if str(guild.id) in log:
            ...
            embed = msg_f({
                ...
                "value":
                [["Member:", f"{user.name} | {user.id}"],
                 ["Unbanned By:", f"{entery.user.name} | {entery.user.id}"]]
            })
            channel = await guild.fetch_channel(log_c)
            await channel.send(embed=embed)
