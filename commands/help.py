...
def bot_admin(ctx):
    # get the admins on the db 
    db = pymongo.MongoClient(getenv["mongoDBclient"])["settings"]
    bot_admin = db["bot_admins"]
    r = bot_admin.find_one({"_id": 0})
    if not r:
        bot_admin.insert_one({"_id": 0, "Admins": ["821486817957642242"]})
        r = bot_admin.find_one({"_id": 0})
    #check if the member is bot admin
...
class MyPageSource(menus.ListPageSource):  # menus.ButtonMenuPages
    ...
    async def format_page(self, menu: menus.ButtonMenuPages, entries):
        ...
        guild = await entries[2].fetch_guild(entries[1])
        db = pymongo.MongoClient(getenv["mongoDBclient"])[str(guild.id)]
        prefix = db["prefix"].find_one({"_id": 0})[entries[1]]
        data = {
            "Home": [{}, "", ""],
            "General Commands": [{
            ...
            "Admin Commands": [{
                ...
                "ban": [
                    "<member> [reason]", "to ban someone", "Ban members perms",
                    [], "922790457556226078"
                ],
                "unban":
                ["<member>", "to unban someone", "Ban members perms", [], ""],
                "add-bad-words": [
                    "[words]",
                ...
                "unmute": [
                    "<member>", "to Cancel the effect of the mute command",
                    "manage messages perms", [], ""
                ],
                "hide": [
                    "<channel>", "Make any channel unvisible to normal people",
                    "manage messages perms",
                    ["prefix-hide room-mention", "prefix-show room-id"], ""
                ],
                "show": [
                    "<channel>", "Make any channel visible to normal people",
                    "manage messages perms",
                    ["prefix-show room-mention", "prefix-show room-id"], ""
                ],
                "nick": [
                    "<member> [nick name]", "to change member nickname",
        ...
        if entries[0] == "Home":
            ...
            embed.add_field(name=f"Welcome to Guardzilla",
                            value=f"The bot's  prefix here is `{prefix}`",
                            inline=False)
            bott = entries[2]
            embed.add_field(
                name=f"_ _",
                value=f"The bot's latency: {round(bott.latency * 1000)}ms",
                inline=False)
            embed.add_field(
                name=f"click the buttons below to see help categories",
                value=f"_ _",
                inline=False)
        else:
            embed = nextcord.Embed(title="Guardzilla's commands",
                                   color=0xA020F0)
            msg = ""
...
class General(commands.Cog):
    ...
    async def help(self, ctx, help_c=None):
        ...
        if help_c is None:
            ...
            try:
                command = help_c
                prefix = str(ctx.message.content).split("help")[0]
