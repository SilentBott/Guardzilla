...
class Ticket(commands.Cog):
    ...
    async def ticket(self, ctx):
        ...
        try:
            category = nextcord.utils.get(ctx.guild.categories,
                                          id=int(ticket[str(ctx.guild.id)][0]))
            if category:
                ticket[str(ctx.guild.id)][0] = str(category.id)
            if not category:
                raise ValueError
        except:
            category = nextcord.utils.get(ctx.guild.categories, name='tickets')
            if category:
                try:
                    ticket[str(ctx.guild.id)][0] = str(category.id)
                except IndexError:
        ...
        if len(ticket[str(ctx.guild.id)]) == 1:
            ...
            role = nextcord.utils.get(ctx.guild.roles,
                                      id=int(ticket[str(ctx.guild.id)][1]))
            if not role:
                role = nextcord.utils.get(ctx.guild.roles,
                                          name="ticket manager")
                if role:
        ...
        if len(ticket[str(ctx.guild.id)]) <= 2:
            ...
            await ctx.send(
                f"New ticket created {channel.mention} | {channel.id}")
            await channel.send("Ticket created.")
        else:
            try:
                if str(ctx.author.id) in ticket[str(ctx.guild.id)][2]:
                    channel = await ctx.guild.fetch_channel(ticket[str(
                        ctx.guild.id)][2][str(ctx.author.id)][0])
                    await ctx.send(
                        f"You already have one {channel.mention} | {channel.id}"
                    )
                else:
                    channel = await ctx.guild.create_text_channel(
                        f"{ctx.author.name}'s ticket",
